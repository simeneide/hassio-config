...
for room in rooms:
    ...
    if final_target_temp is not None:
        id = "climate." + room
        hass.services.call(domain = "climate", service = "set_temperature", 
            service_data = {"temperature" : final_target_temp, "entity_id" : id
        })
        #hass.states.set(id, hass.states.get(id).state, {"temperature" : final_target_temp})
